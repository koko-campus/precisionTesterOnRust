...
fn main() {
...

    {
        ...
        // f256 prec of rug
        let prec256 = 256;
        let mut f256_prec_rug = Float::with_val(prec256, 10) / Float::with_val(prec256, 3);
        ...
        println!("f512 of rug -> \"{}\"", f512_prec_rug.clone()); // 3.33333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333323

        // 異なる精度のデータ型間の演算
        println!("calc between different data type -> {}", f256_prec_rug.clone() + f512_prec_rug.clone()); // 6.666666666666666666666666666666666666666666666666666666666666666666666666666644
        // -> 小さいほうへ、、、

        // 「setPrec」で精度を変換しようね♪
        f256_prec_rug.set_prec(prec512);
        println!("calc between different data type(set_prec) -> {}", f256_prec_rug + f512_prec_rug); // 6.66666666666666666666666666666666666666666666666666666666666666666666666666665515177525987407383281819751626613390517866618075162481996839537310854426244970
    }
